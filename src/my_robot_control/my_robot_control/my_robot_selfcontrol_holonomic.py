...
class RobotSelfControl(Node):
    ...
    def laser_callback(self, scan):
        ...
        # React to obstacle: move in holonomic opposite direction of detected wall using persistent state (FSM)
        if closest_distance < self._distanceLimit:

            # Change mode based on obstacle position
            # FRONT wall -> move backward
            if zone == "FRONT":
                self._current_mode = "BACKWARD"

            # LEFT wall -> move right
            elif zone == "LEFT":
                self._current_mode = "MOVE_RIGHT"
            
            # RIGHT wall -> move left
            elif zone == "RIGHT":
                self._current_mode = "MOVE_LEFT"

            # BACK wall -> move forward
            elif zone in ["BACK_LEFT", "BACK_RIGHT"]:
                self._current_mode = "FORWARD"

        else:
            # No obstacle: keep current mode
            pass
            
        # Now decide the motion based on the current mode

        self._msg.angular.z = 0.0 # No rotation in holonomic avoidance

        if self._current_mode == "FORWARD":
            self._msg.linear.x = self._forwardSpeed * self._speedFactor
            self._msg.linear.y = 0.0

        elif self._current_mode == "MOVE_LEFT":
            self._msg.linear.x = 0.0
            self._msg.linear.y = self._forwardSpeed * self._speedFactor

        elif self._current_mode == "MOVE_RIGHT":
            self._msg.linear.x = 0.0
            self._msg.linear.y = -self._forwardSpeed * self._speedFactor
